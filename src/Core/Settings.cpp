...
Idleness timeout for sending and receiving data to/from S3. Fail if a single TCP read or write call blocks for this long.
    ...
    DECLARE(UInt64, s3_no_such_key_max_retry_attempts, S3::DEFAULT_NO_SUCH_KEY_MAX_RETRY_ATTEMPTS, R"(
Idleness timeout for sending and receiving data to/from S3. Fail if a single TCP read or write call blocks for this long.
)", 0) \
    DECLARE(UInt64, s3_no_such_key_initial_retry_backoff_ms, S3::DEFAULT_NO_SUCH_KEY_INITIAL_RETRY_BACKOFF_MS, R"(
Idleness timeout for sending and receiving data to/from S3. Fail if a single TCP read or write call blocks for this long.
)", 0) \
    DECLARE(UInt64, s3_no_such_key_max_retry_backoff_ms, S3::DEFAULT_NO_SUCH_KEY_MAX_RETRY_BACKOFF_MS, R"(
Idleness timeout for sending and receiving data to/from S3. Fail if a single TCP read or write call blocks for this long.
)", 0) \
