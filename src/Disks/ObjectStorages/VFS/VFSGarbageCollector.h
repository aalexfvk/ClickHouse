...
struct SnapshotMetadata
{
    ...
    SnapshotMetadata(
        const String & object_storage_key_, uint64_t metadata_version_ = 0ull, uint64_t total_size_ = 0ull, int32_t znode_version_ = -1)
        : metadata_version(metadata_version_)
        , object_storage_key(object_storage_key_)
        , total_size(total_size_)
        , znode_version(znode_version_)
...
private:
    ...
    std::pair<std::unique_ptr<WriteBuffer>, StoredObject> getShapshotWriteBufferAndSnaphotObject(const SnapshotMetadata & snapshot_meta) const;
