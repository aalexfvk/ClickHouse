...
# Fetch the target branch so that EXPECTED_SHA (extracted from the candidate
# branch name) is known to the local repository.  git push --force-with-lease
# resolves the expected SHA locally, so without this fetch it fails with
# "cannot parse expected object name" when the SHA is not yet in the local
# object store.
